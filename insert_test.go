...
func TestInsertBuilderOutput(t *testing.T) {
	b := Insert("table").Columns("field1").Values(1).Output("inserted.ID")

	sql, args, err := b.ToSql()
	assert.NoError(t, err)

	expectedSql := "INSERT INTO table (field1) OUTPUT inserted.ID VALUES (?)"
	assert.Equal(t, expectedSql, sql)
